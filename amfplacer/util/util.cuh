...
#define allocateCUDA(var, size)                                                                                        \
  {                                                                                                                    \
    ...
    amfplacerAssertMsg(status == cudaSuccess, "cudaMalloc failed for " #var);                                           \
  }

#define destroyCUDA(var)                                                                                               \
  {                                                                                                                    \
    cudaError_t status = cudaFree(var);                                                                                \
    amfplacerAssertMsg(status == cudaSuccess, "cudaFree failed for " #var);                                             \
  }
...
#define memsetCUDA(var, value, size)                                                                                   \
  {                                                                                                                    \
    ...
    amfplacerAssertMsg(status == cudaSuccess, "cudaMemset failed for " #var);                                           \
  }

#define checkCUDA(status)                                                                                              \
  { amfplacerAssertMsg(status == cudaSuccess, "CUDA Runtime Error %s", cudaGetErrorString(status)); }
