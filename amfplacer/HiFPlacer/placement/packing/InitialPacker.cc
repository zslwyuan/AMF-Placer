 ...
 * @brief  This implementation file contains APIs' implementation of the InitialPacker which identifies macros from the
 * design netlist based on pattern matching
 * @version 0.1
 * @date 2021-10-02
 *
...
void InitialPacker::pack()
{
    ...
    if (JSONCfg.find("unpredictableMacroFile") != JSONCfg.end())
    {
        loadOtherCLBMacros(JSONCfg["unpredictableMacroFile"]);
    }
    ...
    if (JSONCfg.find("fixedUnitsFile") != JSONCfg.end())
    {
        loadFixedPlacementUnits(JSONCfg["fixedUnitsFile"]);
    }
    else
    {
        print_error(
...
void InitialPacker::enhanceIONets()
{
    for (auto net : placementInfo->getPlacementNets())
    {
        if (net->getDriverUnits().size() == 1)
        {
            ...
            {
                net->getDesignNet()->enhanceOverallClusterNetEnhancement(1.5);
            }
        }
