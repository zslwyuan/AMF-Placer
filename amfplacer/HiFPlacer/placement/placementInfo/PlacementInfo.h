...
class PlacementInfo
{
  public:
    ...
    inline void setDSPInnerDelay()
    {
        simplePlacementTimingInfo->setDSPInnerDelay();
    }
    ...
    /**
     * @brief clear attributes which can not be recorded
     *
     */
    inline void clearSomeAttributesCannotRecord()
    {
        for (auto PU : getPlacementUnits())
        {
            if (PU->isPacked())
                PU->resetPacked();
        }
        for (auto pair : getPULegalXY().first)
        {
...
#pragma omp parallel for
    ...
    /**
     * @brief clear the PlacementUnit Mapping to clock region centers for timing optimzation
     *
     */
    void clearPU2ClockRegionCenters()
    {
