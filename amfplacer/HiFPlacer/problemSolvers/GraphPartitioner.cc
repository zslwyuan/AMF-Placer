    ...
    int eachClusterBRAMNum)
{
    ...
    // print_info("cut == " + std::to_string(cut));
    ...
    // recursiveMinCutPartition(outputClusters[0], graphPartitioner, eachClusterDSPNum, eachClusterBRAMNum);
    // recursiveMinCutPartition(outputClusters[1], graphPartitioner, eachClusterDSPNum, eachClusterBRAMNum);
    // pid_t pid = fork();
    // if (pid > 0) {
    //     print_info("parent id " + std::to_string(pid));
    //     recursiveMinCutPartition(outputClusters[0], graphPartitioner, eachClusterDSPNum, eachClusterBRAMNum);
    // } else if (pid == 0) {
    //     print_info("child id " + std::to_string(pid));
    //     recursiveMinCutPartition(outputClusters[1], graphPartitioner, eachClusterDSPNum, eachClusterBRAMNum);
    // }
...
template <class NodeList, class NetList>
ExternalProcessFunc * 
GraphPartitioner<NodeList, NetList>::partitionHyperGraph(const std::vector<int> &inputCluster,
//int* GraphPartitioner<NodeList, NetList>::partitionHyperGraph(const std::vector<int> &inputCluster,
                        ...
                        int* &partvec, int* &partweights, int* &cut)
{
    ...
    unsigned int memorySize = numHyperNodes + numPlacementNets + 1 + numPlacementPins + numHyperNodes + 2 + 1;
    
    ExternalProcessFunc *externalProc =
       new ExternalProcessFunc("", memorySize * 4, graphPartitioner->verbose);
    int* cwghts = (int *)externalProc->getSharedMemory();
    //int* cwghts = (int*) malloc(memorySize * sizeof(int));
    int* xpins = cwghts + numHyperNodes;
    int* pins = xpins + numPlacementNets + 1;
    partvec = pins + numPlacementPins;
    partweights = partvec + numHyperNodes; // 6 + numHyperNodes + numPlacementNets + 1 + numPlacementPins +
    cut = partweights + 2;
...
template <class NodeList, class NetList>
ExternalProcessFunc* GraphPartitioner<NodeList, NetList>::partitionHyperGraphByShareMem(const std::vector<int> &inputCluster,
                        GraphPartitioner<NodeList, NetList> *graphPartitioner, const double& final_imbal, 
                        int& numHyperNodes, int& numPlacementNets, int& numPlacementPins, 
                        int* &partvec, int* &partweights, int* &cut)
{
    ...
    *(double *)(shareMemory + 3) = final_imbal;
    ...
    int *pins = xpins + numPlacementNets + 1;
    partvec = pins + numPlacementPins;
    partweights = partvec + numHyperNodes;
...
//     ExternalProcessFunc *ptrProc =
//        new ExternalProcessFunc(targetPath, shareMemorySize * 4, graphPartitioner->verbose);
//     int *shareMemory = (int *)ptrProc->getSharedMemory();
//     shareMemory[0] = numHyperNodes;
//     shareMemory[1] = numPlacementNets;
//     shareMemory[2] = numPlacementPins;
...
//     // ------------------
    ...
    int *partvec = nullptr;
    int *partweights = nullptr;
    int *cut = nullptr;
    auto ptrProc = partitionHyperGraph(inputCluster, graphPartitioner, final_imbal, 
                       numHyperNodes, numPlacementNets, numPlacementPins,
                       partvec, partweights, cut);
    // auto ptrProc = partitionHyperGraphByShareMem(inputCluster, graphPartitioner, final_imbal, 
    //                   numHyperNodes, numPlacementNets, numPlacementPins,
    //                   partvec, partweights, cut);
