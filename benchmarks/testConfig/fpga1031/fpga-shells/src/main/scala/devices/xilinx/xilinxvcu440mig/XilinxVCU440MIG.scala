...
import sifive.fpgashells.ip.xilinx.vcu440mig.{VCU440MIGIOClocksReset, VCU440MIGIODDR, vcu440mig}

case class XilinxVCU440MIGParams(
  address : Seq[AddressSet]
)

class XilinxVCU440MIGPads(depth : BigInt) extends VCU440MIGIODDR(depth) {
  def this(c : XilinxVCU440MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxVCU440MIGIO(depth : BigInt) extends VCU440MIGIODDR(depth) with VCU440MIGIOClocksReset

class XilinxVCU440MIGIsland(c : XilinxVCU440MIGParams)(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  val crossing = AsynchronousCrossing(8)
  require((depth<=0x100000000L),"vcu440mig supports upto 4GB depth configuraton")
  ...
  lazy val module = new LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxVCU440MIGIO(depth)
    })

    //MIG black box instantiation
    val blackbox = Module(new vcu440mig(depth))
...
class XilinxVCU440MIG(c : XilinxVCU440MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxVCU440MIGIsland(c))
