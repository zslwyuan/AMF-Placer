...
package sifive.fpgashells.ip.xilinx.vcu440mig
...
// IP VLNV: xilinx.com:customize_ip:vcu440mig:1.0
// Black Box

class VCU440MIGIODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
  require((depth<=0x100000000L),"VCU440MIGIODDR supports upto 4GB depth configuraton")
...
//reused directly in io bundle for sifive.blocks.devices.xilinxvcu440mig
trait VCU440MIGIOClocksReset extends Bundle {
  //inputs
...
class vcu440mig(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  require((depth<=0x100000000L),"vcu440mig supports upto 4GB depth configuraton")

  val io = new VCU440MIGIODDR(depth) with VCU440MIGIOClocksReset {
