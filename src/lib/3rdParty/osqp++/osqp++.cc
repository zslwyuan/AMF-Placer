...
absl::Status CheckDimensions(const int left_value, const int right_value, absl::string_view left_name,
                             absl::string_view right_name)
{
    if (left_value != right_value)
    {
        return absl::InvalidArgumentError(absl::StrCat("Dimension mismatch: ", left_name, " (= ", left_value,
...
absl::Status OsqpSolver::Init(const OsqpInstance &instance, const OsqpSettings &settings, bool MKLorNot)
{
    if (!instance.objective_matrix.isCompressed())
    {
        return absl::InvalidArgumentError("objective_matrix must be compressed (call makeCompressed()).");
    }
    if (!instance.constraint_matrix.isCompressed())
    {
        return absl::InvalidArgumentError("constraint_matrix must be compressed (call makeCompressed()).");
    }
    ...
    ::csc objective_matrix = {objective_matrix_upper_triangle.outerIndexPtr()[num_variables],
                              num_variables,
                              num_variables,
    ...
    ::csc constraint_matrix = {instance.constraint_matrix.outerIndexPtr()[num_variables],
                               num_constraints,
                               num_variables,
    ...
    if (MKLorNot)
        osqp_settings.linsys_solver = MKL_PARDISO_SOLVER;
    ...
    case OSQP_DATA_VALIDATION_ERROR:
        return absl::InvalidArgumentError("Unable to initialize OSQP: data validation error.");
    case OSQP_SETTINGS_VALIDATION_ERROR:
        return absl::InvalidArgumentError("Unable to initialize OSQP: invalid settings.");
    case OSQP_LINSYS_SOLVER_LOAD_ERROR:
        // This should never happen because qdldl is statically linked in.
        return absl::UnknownError("Unable to initialize OSQP: unable to load linear solver.");
    case OSQP_LINSYS_SOLVER_INIT_ERROR:
        return absl::UnknownError("Unable to initialize OSQP: unable to initialize linear solver.");
    case OSQP_NONCVX_ERROR:
        return absl::InvalidArgumentError("Unable to initialize OSQP: the problem appears non-convex.");
    case OSQP_MEM_ALLOC_ERROR:
        return absl::UnknownError("Unable to initialize OSQP: memory allocation error.");
    case OSQP_WORKSPACE_NOT_INIT_ERROR:
        return absl::UnknownError("Unable to initialize OSQP: workspace not initialized.");
    }
    return absl::UnknownError("Unable to initialize OSQP: unrecognized error code.");
}

namespace
{

absl::Status VerifySameSparsity(const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> &new_matrix,
                                const csc *ref_matrix, size_t num_variables)
{
    if (new_matrix.nonZeros() != ref_matrix->p[num_variables])
    {
        return absl::InvalidArgumentError("The new new matrix should have the same number of non-zero "
                                          "elements.");
    }
    ...
    for (size_t i = 0; i < num_variables; ++i)
    {
        ...
        {
            return absl::InvalidArgumentError("Sparsity of the new matrix differs from the previously "
                                              "defined matrix.");
        }
    }
    for (int i = 0; i < new_matrix.innerSize(); ++i)
    {
    ...
    OSQPWorkspaceHelper *workspace)
{
    ...
    if (objective_matrix_upper_triangle.rows() != objective_matrix_upper_triangle.cols() ||
        objective_matrix_upper_triangle.rows() != num_variables)
    {
        return absl::InvalidArgumentError(absl::StrFormat(
            ...
            objective_matrix_upper_triangle.rows(), objective_matrix_upper_triangle.cols(), num_variables));
    }

    OSQP_RETURN_IF_ERROR(VerifySameSparsity(objective_matrix_upper_triangle, workspace->data->P, num_variables));

    c_int nnzP = objective_matrix_upper_triangle.nonZeros();

    const int return_code = osqp_update_P(workspace, objective_matrix_upper_triangle.valuePtr(), OSQP_NULL, nnzP);
    if (return_code == 0)
    {
        return absl::OkStatus();
    }
    return absl::UnknownError("Unable to update OSQP P matrix: unrecognized error code.");
}
...
absl::Status UpdateUpperTriangularObjectiveMatrixAndConstraintMatrix(
    const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> &objective_matrix_upper_triangle,
    const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> &constraint_matrix, OSQPWorkspaceHelper *workspace)
{
    const c_int num_variables = workspace->data->n;
    ...
    if (constraint_matrix.cols() != num_variables)
    {
        return absl::InvalidArgumentError(
            absl::StrFormat("The new constraint matrix should column size equal to the "
                            "number of variables. Matrix dimensions: %d x %d, num_variables=%d",
    ...
    const int return_code = osqp_update_P_A(workspace, objective_matrix_upper_triangle.valuePtr(), OSQP_NULL, nnzP,
                                            constraint_matrix.valuePtr(), OSQP_NULL, nnzA);
    if (return_code == 0)
    {
        return absl::OkStatus();
    }
    return absl::UnknownError("Unable to update OSQP P and A matrix: unrecognized error code.");
}
...
bool IsUpperTriangular(const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> &matrix)
{
    ...
    {
        for (Eigen::SparseMatrix<double, Eigen::ColMajor, c_int>::InnerIterator it(matrix, i); it; ++it)
        {
            if (it.col() < it.row())
            {
                return false;
...
absl::Status
OsqpSolver::UpdateObjectiveMatrix(const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> &objective_matrix)
{
...
absl::Status
OsqpSolver::UpdateConstraintMatrix(const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> &constraint_matrix)
{
    ...
    const int return_code = osqp_update_A(workspace_.get(), constraint_matrix.valuePtr(), OSQP_NULL, nnzA);
    if (return_code == 0)
    {
        return absl::OkStatus();
    }
    return absl::UnknownError("Unable to update OSQP A matrix: unrecognized error code.");
}
    ...
    const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> &constraint_matrix)
{
    ...
    if (IsUpperTriangular(objective_matrix))
    {
        return UpdateUpperTriangularObjectiveMatrixAndConstraintMatrix(objective_matrix, constraint_matrix,
                                                                       workspace_.get());
    }
    ...
    Eigen::SparseMatrix<double, Eigen::ColMajor, c_int> objective_matrix_upper_triangle =
        objective_matrix.triangularView<Eigen::Upper>();
    return UpdateUpperTriangularObjectiveMatrixAndConstraintMatrix(objective_matrix_upper_triangle, constraint_matrix,
                                                                   workspace_.get());
}
...
Map<const VectorXd> OsqpSolver::primal_infeasibility_certificate() const
{
    ...
    OSQP_CHECK(exit_code == OsqpExitCode::kPrimalInfeasible || exit_code == OsqpExitCode::kPrimalInfeasibleInaccurate);
    return Map<const VectorXd>(workspace_->delta_y, workspace_->data->m);
}

absl::Status OsqpSolver::SetWarmStart(const Ref<const VectorXd> &primal_vector, const Ref<const VectorXd> &dual_vector)
{
...
absl::Status OsqpSolver::SetPrimalWarmStart(const Ref<const VectorXd> &primal_vector)
{
    if (!IsInitialized())
    {
...
absl::Status OsqpSolver::SetDualWarmStart(const Ref<const VectorXd> &dual_vector)
{
    if (!IsInitialized())
    {
        return absl::FailedPreconditionError("OsqpSolver is not initialized.");
...
absl::Status OsqpSolver::SetObjectiveVector(const Ref<const VectorXd> &objective_vector)
{
    if (!IsInitialized())
    {
        return absl::FailedPreconditionError("OsqpSolver is not initialized.");
...
absl::Status OsqpSolver::SetBounds(const Ref<const VectorXd> &lower_bounds, const Ref<const VectorXd> &upper_bounds)
{
    if (!IsInitialized())
    {
        return absl::FailedPreconditionError("OsqpSolver is not initialized.");
    ...
    for (int i = 0; i < num_constraints; i++)
    {
        ...
        {
            return absl::InvalidArgumentError(absl::StrCat("Inconsistent bounds at index ", i, ", ", lower_bounds[i],
                                                           " must be <= ", upper_bounds[i], "."));
        }
    }

    const int return_code = osqp_update_bounds(workspace_.get(), lower_bounds.data(), upper_bounds.data());
    if (return_code != 0)
    {
...
absl::Status OsqpSolver::UpdateMaxIter(int max_iter_new)
{
    ...
    if (max_iter_new <= 0)
    {
        return absl::InvalidArgumentError(absl::StrCat("Invalid max_iter value: ", max_iter_new));
    }
...
absl::Status OsqpSolver::UpdateEpsAbs(double eps_abs_new)
{
    ...
    if (eps_abs_new <= 0.0)
    {
        return absl::InvalidArgumentError(absl::StrCat("Invalid eps_abs value: ", eps_abs_new));
    }
...
absl::Status OsqpSolver::UpdateTimeLimit(double time_limit_new)
{
    ...
    {
        return absl::InvalidArgumentError(absl::StrCat("Invalid time_limit value: ", time_limit_new));
    }
