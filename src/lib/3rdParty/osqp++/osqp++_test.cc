...
void ExpectElementsAre(Eigen::Map<const Eigen::VectorXd> vec, absl::Span<const double> expected, const double tolerance)
{
    EXPECT_EQ(vec.size(), expected.size());
    for (int i = 0; i < vec.size(); ++i)
    {
        EXPECT_THAT(vec[i], DoubleNear(expected[i], tolerance)) << "violation at index =" << i;
    }
}
...
TEST(OsqpTest, DimensionMismatchObjectiveMatrix)
{
    ...
    EXPECT_EQ(solver.Init(instance, settings).code(), absl::StatusCode::kInvalidArgument);
...
class ParameterizedTwoDimensionalQpTest : public testing::WithParamInterface<bool>, public TwoDimensionalQpTest
{
};
TEST_P(ParameterizedTwoDimensionalQpTest, SolvesWithNewObjectiveMatrix)
{
    ...
    EXPECT_NEAR(solver_.objective_value(), x_exp * x_exp + 0.5 * x_exp * y_exp + y_exp * y_exp, 2 * kTolerance);
    ...
    x_exp = 1.0;
    y_exp = -0.5;
    EXPECT_NEAR(solver_.objective_value(), x_exp * x_exp + 0.5 * x_exp * y_exp + 0.5 * y_exp * y_exp, 2 * kTolerance);
    ExpectElementsAre(solver_.primal_solution(), {x_exp, y_exp}, kTolerance);
}
...
INSTANTIATE_TEST_SUITE_P(UseUpperTriangularObjectiveMatrix, ParameterizedTwoDimensionalQpTest,
