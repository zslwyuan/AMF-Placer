...
template <class PointT> class KDTree
{
  public:
    ...
    KDTree(const std::vector<PointT> &points, float y2xRatio) : root_(nullptr), y2xRatio(y2xRatio)
    {
        build(points);
    }
  ...
  private:
    ...
    double distance(const PointT &p, const PointT &q) const
    {
        double dist = 0;
        for (size_t i = 0; i < PointT::DIM; i++)
        {
            if (i == 1)
                dist += y2xRatio * std::fabs(p[i] - q[i]);
            else
    ...
    void knnSearchRecursive(const PointT &query, const Node *node, KnnQueue &queue, int k) const
    {
        ...
        const double diff =
            axis == 1 ? (y2xRatio * fabs(query[axis] - train[axis])) : (fabs(query[axis] - train[axis]));
