...
QBLCanvas::QBLCanvas() : _rendererType(RendererBlend2D), _dirty(true), _fps(0), _frameCount(0)
{
    _elapsedTimer.start();
...
void QBLCanvas::keyPressEvent(QKeyEvent *event)
{
    if (onKeyPressEvent)
        onKeyPressEvent(event);
}

void QBLCanvas::keyReleaseEvent(QKeyEvent *event)
{
...
{
    if (_rendererType == RendererBlend2D)
    {
        if (onRenderB2D)
        {
    ...
    {
        assert(false && "we don't support other rendering currently.");
    }
...
{
    static const uint32_t rendererTypes[] = {RendererBlend2D};
